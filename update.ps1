...
function global:au_GetLatest {
    ...
    if ($download_page.Content -match 'Version\s+([0-9.]+)\s+Basic') {
        $version = $matches[1]
    } else {
    ...
    if ($majorGeeksLink) {
        ...
        $url = ($mgPage.Links | Where-Object { $_.href -match 'files3\.codecguide\.com/.*Basic\.exe' } | Select-Object -First 1).href
    } else {
        $url = ($download_page.Links | Where-Object { $_.href -match 'files3\.codecguide\.com/.*Basic\.exe' -and $_.href -notmatch 'old' } | Select-Object -First 1).href
    }
    if (-not $url) {
